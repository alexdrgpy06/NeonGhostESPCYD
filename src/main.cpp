...
void drawSingleButton(int x, const char* label, bool pressed) {
    uint16_t mainColor = petStats.getStageColor();
    uint16_t bg = pressed ? mainColor : C_DARK;
    uint16_t fg = pressed ? C_DARK : mainColor;
    
    tft.fillRoundRect(x, BUTTON_Y, BUTTON_W, BUTTON_H, 6, bg);
    tft.drawRoundRect(x, BUTTON_Y, BUTTON_W, BUTTON_H, 6, mainColor);
    
    tft.setTextColor(fg, bg);
    tft.setTextSize(1);
    
    // Center text
    int textWidth = strlen(label) * 6; // Approx 6px per char
