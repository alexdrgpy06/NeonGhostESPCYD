...
void drawButtons(int pressedBtn = -1);
void drawTopBar();
void drawStatusBar();
void drawGhostArea();
void fullReset();
...
void drawSingleButton(int x, int y, int w, int h, const char* label, uint16_t color, bool pressed) {
    ...
    int textWidth = strlen(label) * 6; // size 1 is 5x7 + 1px spacing = 6px per char
    int textX = x + (w - textWidth) / 2;
    int textY = y + (h - 8) / 2; // size 1 is 8px high
    
    tft.setCursor(textX, textY);
    tft.print(label);
}

void drawButtons(int pressedBtn) {
    uint16_t btnColor = petStats.getStageColor();
    int x1 = 15, x2 = 85, x3 = 155;

    drawSingleButton(x1, BUTTON_Y, BUTTON_W, BUTTON_H, "FEED", btnColor, pressedBtn == 0);
    drawSingleButton(x2, BUTTON_Y, BUTTON_W, BUTTON_H, "ATTACK", btnColor, pressedBtn == 1);
    drawSingleButton(x3, BUTTON_Y, BUTTON_W, BUTTON_H, "MENU", btnColor, pressedBtn == 2);
}
