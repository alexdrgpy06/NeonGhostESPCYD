...
void drawSingleButton(int x, int y, int w, int h, const char* label, uint16_t color, bool pressed) {
    ...
    // Center dynamically: text width is roughly strlen(label) * 6 (for text size 1)
    int textWidth = 0;
    for(int i=0; label[i] != '\0'; i++) textWidth += 6;

    int cx = x + (w - textWidth) / 2;
    int cy = y + (h - 8) / 2; // Assuming height 8 for text size 1
