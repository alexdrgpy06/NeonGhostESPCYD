...
void drawMainButton(int x, const char* label, bool pressed) {
    ...
    uint16_t bgColor = pressed ? themeColor : C_DARK;
    ...
    // Center text (approx 6px per char width)
    int textWidth = strlen(label) * 6;
    int textX = x + (BUTTON_W - textWidth) / 2;
    int textY = BUTTON_Y + 18; // Vertically centered
...
void drawButtons() {
    ...
    drawMainButton(x1, "FEED");
    drawMainButton(x2, "ATTACK");
    drawMainButton(x3, "MENU");
}
...
void handleTouch(int tx, int ty) {
    ...
    if (ty >= BUTTON_Y) {
        if (tx >= 15 && tx < 85) {
            ...
            delay(50);
            // No need to redraw unpressed, menu will cover it
