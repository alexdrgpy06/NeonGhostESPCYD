...
void drawButtons();
void drawSingleButton(int x, int y, int w, int h, const char* label, uint16_t color, bool pressed = false);
void drawTopBar();
void drawStatusBar();
void drawGhostArea();
void fullReset();
...
void drawSingleButton(int x, int y, int w, int h, const char* label, uint16_t color, bool pressed) {
    uint16_t bgColor = pressed ? color : C_DARK;
    uint16_t fgColor = pressed ? C_DARK : color;
    ...
    int textWidth = strlen(label) * 6; // Approx width for size 1
    int tx = x + (w - textWidth) / 2;
    int ty = y + (h - 8) / 2; // Approx height is 8 for size 1
...
void drawButtons() {
    ...
    drawSingleButton(x1, BUTTON_Y, BUTTON_W, BUTTON_H, "FEED", btnColor);
    drawSingleButton(x2, BUTTON_Y, BUTTON_W, BUTTON_H, "ATTACK", btnColor);
    drawSingleButton(x3, BUTTON_Y, BUTTON_W, BUTTON_H, "MENU", btnColor);
}
...
void handleTouch(int tx, int ty) {
    ...
    if (ty >= BUTTON_Y && ty <= BUTTON_Y + BUTTON_H) {
        uint16_t btnColor = petStats.getStageColor();
        if (tx >= 15 && tx < 85) {
            // FEED - Visual Feedback
            drawSingleButton(15, BUTTON_Y, BUTTON_W, BUTTON_H, "FEED", btnColor, true);
            delay(50);
            drawSingleButton(15, BUTTON_Y, BUTTON_W, BUTTON_H, "FEED", btnColor, false);
            ...
            drawSingleButton(85, BUTTON_Y, BUTTON_W, BUTTON_H, "ATTACK", btnColor, true);
            delay(50);
            drawSingleButton(85, BUTTON_Y, BUTTON_W, BUTTON_H, "ATTACK", btnColor, false);

            // Execute random available ability
            ...
            drawSingleButton(155, BUTTON_Y, BUTTON_W, BUTTON_H, "MENU", btnColor, true);
            delay(50);
            // Draw unpressed state, though drawMenu will immediately overwrite
            drawSingleButton(155, BUTTON_Y, BUTTON_W, BUTTON_H, "MENU", btnColor, false);
