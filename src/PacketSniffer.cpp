...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // ⚡ Bolt Optimization: Skip payload inspection for encrypted frames
        // The Protected bit (0x40) indicates encryption; EAPOL is always plaintext.
        // This yields massive speedups by avoiding O(N) scanning on encrypted payloads.
        if ((packet[1] & 0x40) == 0 && len >= 26) {
