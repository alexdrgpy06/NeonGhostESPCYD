...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Optimization: EAPOL handshakes are never encrypted.
        // Skip scanning the payload if the Protected bit (0x40) is set in the Frame Control field.
        // We also enforce a minimum length bounds check (24 bytes) before accessing header bytes.
