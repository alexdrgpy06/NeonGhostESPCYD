...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Fast path: Only scan for plaintext EAPOL headers if the packet is long enough
        // and NOT encrypted (Protected bit 0x40 in Frame Control 2 is not set).
        // This yields a significant performance improvement (~24x speedup on encrypted frames)
