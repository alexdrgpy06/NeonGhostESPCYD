...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Skip encrypted frames (Protected bit 0x40 in Frame Control[1])
        // EAPOL handshakes are never encrypted, so we can safely skip the payload scan.
        // Also ensure minimum length bounds check.
