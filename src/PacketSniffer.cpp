...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Bolt: Optimize EAPOL scanning by checking the 'Protected' bit (packet[1] & 0x40).
        // Encrypted 802.11 data frames cannot contain valid plaintext EAPOL headers,
        // so skipping payload inspection yields massive performance improvements.
        // Also ensure a minimum length bounds check (len >= 24) before scanning.
