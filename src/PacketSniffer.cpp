...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Skip parsing if frame is too short or if it's encrypted (Protected bit is set: packet[1] & 0x40).
        // This gives ~25x performance improvement on encrypted traffic (905ms -> 36ms).
