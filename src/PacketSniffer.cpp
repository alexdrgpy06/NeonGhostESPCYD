...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Optimization: Skip encrypted frames (Protected bit is set in frame control byte 1)
        // Encrypted frames cannot contain valid plaintext EAPOL headers, so checking them wastes CPU.
