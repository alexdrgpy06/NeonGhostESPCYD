...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Optimization: Encrypted frames cannot contain valid plaintext EAPOL headers.
        // Skipping payload inspection for protected frames yields massive performance improvements.
        bool isProtected = (packet[1] & 0x40) != 0;
        if (!isProtected && len >= 24) {
