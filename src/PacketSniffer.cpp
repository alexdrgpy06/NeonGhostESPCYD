...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // ⚡ Bolt: Skip encrypted 802.11 data frames (Protected bit 0x40) to prevent
        // unnecessary payload inspection. Encrypted frames cannot contain valid
        // plaintext EAPOL headers.
        if (len >= 26 && !(packet[1] & 0x40)) {
