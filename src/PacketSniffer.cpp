...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // OPTIMIZATION: Skip encrypted data frames (Protected bit 0x40 set in frame control)
        // Encrypted frames cannot contain valid plaintext EAPOL headers here.
        // Also ensure minimum length before accessing header bytes.
