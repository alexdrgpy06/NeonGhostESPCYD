...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Optimization: Skip payload inspection if frame is encrypted (Protected bit 0x40 is set)
        // or if it's too short to contain a valid EAPOL header. Plaintext EAPOL happens before encryption.
