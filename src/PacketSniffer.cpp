...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Optimization: Skip encrypted frames (Protected bit 0x40 set in frameControl)
        // because they cannot contain valid unencrypted EAPOL headers.
        // Also ensure minimum length. This avoids ~21x linear scan overhead on encrypted data.
