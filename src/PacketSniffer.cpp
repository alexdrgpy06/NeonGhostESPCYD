...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // Skip encrypted frames (Protected bit set) or frames too short to hold headers.
        // Encrypted frames cannot contain plaintext EAPOL headers, so skipping them saves
        // an expensive payload scan (~40-47x speedup).
