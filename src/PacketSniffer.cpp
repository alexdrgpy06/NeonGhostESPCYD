...
void PacketSniffer::processPacket(uint8_t *packet, uint16_t len) {
    ...
    // DATA FRAMES (Type 2) - EAPOL Detection
        ...
        // OPTIMIZATION: Check the Protected bit (bit 6 of frame control byte 1)
        // If the frame is encrypted, we can't inspect the payload for EAPOL (0x888E).
        // Skip the linear scan for a huge performance boost on busy networks.
        // Also ensure minimum length bounds check (24 bytes for 802.11 header) before scanning.
        // We use standard if blocks instead of early returns to ensure downstream logic runs.
        if ((packet[1] & 0x40) == 0 && len >= 24) {
