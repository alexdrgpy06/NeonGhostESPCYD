...
// Simulate PacketSniffer context
struct PacketSniffer {
        ...
        if (type == 2) {
            for (int i = 24; i < len - 6 && i < 60; i++) {
                if (packet[i] == 0x88 && packet[i + 1] == 0x8E) {
                    ...
                    // In real code: pendingEvent = EVT_HANDSHAKE;
                    // In real code: eventDetails = "WPA HANDSHAKE";
                    // In real code: savePacket = true;
                    break;
                }
            }
        }
    ...
    void processPacket_Optimized(uint8_t *packet, uint16_t len) {
        packetCount++;
        if (len < 36) return; // Minimum length for EAPOL (Header + LLC + EAPOL)
